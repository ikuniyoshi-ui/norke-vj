...
const Visualizer: React.FC<VisualizerProps> = ({ manualState }) => {
  ...
  useEffect(() => {
    ...
    // --- PATTERN FACTORY (20 DEFINITIONS) ---
    const createPatterns = () => {
      const p: Record<string, THREE.Object3D> = {};
      const mat = (color: number, wire = true) => new THREE.MeshBasicMaterial({ color, wireframe: wire, transparent: true, opacity: 0.8, blending: THREE.AdditiveBlending });
      ...
      for(let i=0; i<30; i++) {
        const r = new THREE.Mesh(new THREE.TorusGeometry(15, 0.1, 12, 100), mat(0xff00ff));
        r.position.z = -i * 10;
        tunnel.add(r);
      }
      p.tunnel = tunnel;
      ...
      p.swarm = new THREE.Points(swarmGeo, new THREE.PointsMaterial({ size: 0.3, color: 0x00ffaa, transparent: true, blending: THREE.AdditiveBlending }));

      p.grid = new THREE.GridHelper(300, 50, 0x444444, 0x222222);

      const pillars = new THREE.Group();
      for(let i=0; i<60; i++) {
        const pillar = new THREE.Mesh(new THREE.BoxGeometry(1.5, 1, 1.5), mat(0x555555, false));
      ...
      for(let i=0; i<80; i++) {
        const bar = new THREE.Mesh(new THREE.BoxGeometry(0.2, 15, 0.2), mat(0x00ff00, false));
      ...
      for(let i=0; i<30; i++) {
        const plane = new THREE.Mesh(new THREE.PlaneGeometry(20, 2), mat(0xffffff, false));
        plane.position.set((Math.random()-0.5)*80, (Math.random()-0.5)*80, (Math.random()-0.5)*80);
        glitch.add(plane);
      }
      p.glitch = glitch;

      p.starfield = new THREE.Points(swarmGeo.clone(), new THREE.PointsMaterial({ size: 0.15, color: 0xffffff, transparent: true }));

      const vortex = new THREE.Group();
      for(let i=0; i<150; i++) {
        const sph = new THREE.Mesh(new THREE.SphereGeometry(0.4), mat(0xffaa00, false));
      ...
      for(let i=0; i<12; i++) {
        const line = new THREE.Mesh(new THREE.PlaneGeometry(160, 1, 50, 1), mat(0x0055ff));
      ...
      for(let i=0; i<15; i++) {
        const r = new THREE.Mesh(new THREE.RingGeometry(i*4, i*4+0.3, 64), mat(0x00ff88, false));
        rings.add(r);
      }
      p.rings = rings;
      ...
      for(let i=0; i<50; i++) {
        const sl = new THREE.Mesh(new THREE.PlaneGeometry(250, 0.1), mat(0xffffff, false));
      ...
      p.feedback = new THREE.Points(swarmGeo.clone(), new THREE.PointsMaterial({ size: 1.0, color: 0x3300ff, transparent: true, blending: THREE.AdditiveBlending }));
    ...
    const animate = () => {
      ...
      // Camera Animation (Synched with state.cameraSpeed)
